...
import rcp.colibri.vars.GlobalVARS;
import rcp.colibri.vars.database.ComboVARS;
...
public abstract class SAFTImport {
	 ...
	 * 		-> Sales Invoices
	 */
	public SAFTImport(boolean importHeader, boolean importCustomers,
	...
	public void process() throws Exception {
		...
		if(importHeader)
			populateEmpresa(header);

		if(importCustomers)
			populateClientes(masterFiles);

		if(importArticles)
			populateArtigos(masterFiles);

		if(importTaxes)
			populateIVA(masterFiles);
	...
	private int getTaxCode(String code) throws Exception {
	...
	private int getEspacoFiscal(String descricao) throws Exception {
	...
	private String filter(String string) throws Exception {
		return string==null ? "" : StringUtils.removeChars(string, GlobalVARS.EXCLUDE_CHARS);
	}
