...
import org.projectocolibri.api.database.mapper.TableFieldFormat;
...
public abstract class SAFTx0101Import implements ISAFTImport {
	...
	/*
	 * (non-Javadoc)
	 * @see org.projectocolibri.rcp.colibri.dao.saft.ISAFTImport
	 */
	@Override
	...
	private int getTipotaxa(String codigo) throws Exception {
		...
		if(codigo.equals("NOR")){
			return Codigosiva.tipotaxa_normal;
		}else if(codigo.equals("RED")){
			return Codigosiva.tipotaxa_reduzida;
		}else if(codigo.equals("INT")){
			return Codigosiva.tipotaxa_intermedia;
		}else if(codigo.equals("ISE")){
			return Codigosiva.tipotaxa_isenta;
		}

		return Codigosiva.tipotaxa_outras;
	...
	private int getEspacofiscal(String codigo) throws Exception {
		...
		if(codigo.equals("PT")){
			return Codigosiva.espacofiscal_continente;
		}else if(codigo.equals("PT-MA")){
			return Codigosiva.espacofiscal_madeira;
		}else if(codigo.equals("PT-AC")){
			return Codigosiva.espacofiscal_acores;
		}

		return Codigosiva.espacofiscal_outros;
	...
	private int getTipoartigo(ProductType.Enum productType) throws Exception {
		...
		case ProductType.INT_S: return Artigos.tipo_servico;
		case ProductType.INT_I: return Artigos.tipo_imposto;
		}return Artigos.tipo_produto;
	...
	private void populateClientes(MasterFiles masterFiles) throws Exception {
		...
		for(Customer customer: masterFiles.getCustomerList()) {
			...
			Entidades entidade=new Entidades(tipoentidade.getCodigo());
	...
	private String parse(String string) throws Exception {
		return string==null ? "" : StringUtils.removeChars(string, TableFieldFormat.EXCLUDE_CHARS);
	}
